...
//! and the watcher's own write NOT being re-processed.
...
fn watcher_handles_create_and_writes_heartbeat() {
    ...
    let ctx = Ctx::new(vec![forge.clone()], Logger { file: Some(log.clone()), tag: None, quiet: true });
    ...
    wait_for("monitoring banner", || fs::read_to_string(&log).map(|s| s.contains("🔍 Monitoring Forge for file events...")).unwrap_or(false));
    ...
    // The write to A.md was ours: the resulting event must be suppressed.
    std::thread::sleep(Duration::from_millis(2500));
    let text = fs::read_to_string(&log).unwrap();
    assert!(!text.contains("Modified: A.md"), "own write re-processed:\n{text}");
}
