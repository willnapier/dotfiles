...
pub trait Exec {
    ...
    fn operation(&self, args: &[&str], _env: &[(String, String)]) -> CmdResult {
        self.run("rustic", args)
    }
}
...
impl Exec for Real {
    fn operation(&self, args: &[&str], env: &[(String, String)]) -> CmdResult {
        match Command::new("rustic")
            .args(args)
...
impl Exec for Fake {
    fn run(&self, program: &str, args: &[&str]) -> CmdResult {
        ...
        self.responses
            .get(&k)
            .cloned()
            .unwrap_or_else(|| CmdResult::failure(127, "unscripted command"))
    }
}

impl CmdResult {
    fn failure_live() -> Self {
        Self {
            exit_code: 127,
            ..Default::default()
        }
