...
fn layout_buzan_root(
    ...
    for (ci, child) in root.children.iter().enumerate() {
        ...
        // Gap from root edge to text start: past the thick organic stroke,
        // then just enough to clear the adjacent branch line.
        ...
        if !child.children.is_empty() {
            ...
            // Fan = half-gap minus buffer. Never exceed available space —
            // let text_gap grow instead if siblings are tightly packed.
            let buffer = 8.0f64.to_radians();
            let l2_max_tilt = (min_l1_half_gap - buffer).clamp(8.0f64.to_radians(), 35.0f64.to_radians());
            ...
            for (ci2, child2) in child.children.iter().enumerate() {
                ...
                let l2_text_gap = l2_stroke_floor.max(l2_angular).min(150.0);
