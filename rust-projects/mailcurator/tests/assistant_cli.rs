...
const BIN: &str = env!("CARGO_BIN_EXE_mailcurator");
...
fn legacy_parser_help_version_and_unknown_command_keep_their_channels() {
    ...
    assert!(String::from_utf8_lossy(&help.stdout).contains("run"));
...
#[test]
fn discovery_bypasses_config_and_llm_permission_initialisation() {
    let out = run(&[
        "--config",
        "/nonexistent-forge-assistant-fixture",
        "--allow-llm",
        "assistant",
        "capabilities",
    ]);
    ...
    assert!(out.stderr.is_empty());
    let value: Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(value["program"], "mailcurator");
}

#[test]
fn malformed_request_after_root_options_does_not_echo() {
    let out = run(&[
        "--config",
        "/nonexistent-forge-assistant-fixture",
        "assistant",
        "synthetic-private-canary-DO-NOT-ECHO",
    ]);
    assert_eq!(out.status.code(), Some(2));
    assert!(out.stderr.is_empty());
    let value: Value = serde_json::from_slice(&out.stdout).unwrap();
