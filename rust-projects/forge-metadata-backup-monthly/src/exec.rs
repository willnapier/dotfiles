//! Command execution behind a trait, so the monthly run can be unit-tested with
//! canned `forge-metadata-backup` / `git` output instead of a live Forge.
//! Same shape as service-health-check's exec.rs plus `run_in` (a working
//! directory for the git calls).
...
pub trait Exec {
    ...
    /// Like `run`, with the child's working directory set.
...
fn finish(out: std::io::Result<std::process::Output>) -> CmdResult {
    match out {
        Ok(o) => CmdResult {
...
/// Canned responses keyed by `"program arg1 arg2 …"` (the directory of a
/// `run_in` call is recorded in `calls` but not part of the key). Anything not
/// scripted returns exit 127 with an empty stdout, so a test that forgets a
/// command fails loudly rather than passing on an accidental empty string.
#[cfg(test)]
#[derive(Default)]
pub struct Fake {
...
impl Exec for Fake {
    ...
    fn run_in(&self, dir: &Path, program: &str, args: &[&str]) -> CmdResult {
        let k = Self::key(program, args);
        self.calls.borrow_mut().push(format!("[{}] {k}", dir.display()));
        self.responses.get(&k).cloned().unwrap_or_else(|| CmdResult::failure(127, "unscripted command"))
    }
}
