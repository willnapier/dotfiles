...
pub fn append_entry(date: &NaiveDate, entry: &str) -> Result<()> {
    ...
    if content.contains("clinical.todo::") {
        eprintln!(
            "Warning: {} already has a clinical.todo:: block, skipping",
            path.display()
        );
        return Ok(());
    }
